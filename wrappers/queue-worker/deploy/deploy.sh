...
refresh_derived() {
  ...
  AOAI_DEPLOYMENT="${AOAI_DEPLOYMENT:-${AZURE_OPENAI_DEPLOYMENT_REASON01:-}}"
  AOAI_API_VERSION="${AOAI_API_VERSION:-${AZURE_OPENAI_API_VERSION:-}}"
  LOG_LEVEL="${LOG_LEVEL:-INFO}"
  PER_REPLICA_CONCURRENCY="${QUEUE_WORKER_PER_REPLICA_CONCURRENCY:-${PER_REPLICA_CONCURRENCY:-1}}"
...
validate_worker_settings() {
  ...
  if [[ -z "${AZURE_OPENAI_DEPLOYMENT_REASON01:-}" ]]; then
    echo "ERROR: AZURE_OPENAI_DEPLOYMENT_REASON01 must be set in $ENV_FILE for queue-worker awreason execution." >&2
    exit 1
  fi
}
...
render_yaml() {
  ...
  local -a explicit_env_names=(
    ...
    "QUEUE_WORKER_PER_REPLICA_CONCURRENCY"
    "SB_NAMESPACE"
    "SB_QUEUE"
    "SB_RESULTS_QUEUE"
    "REPORT_MODE"
  ...
  append_replacement "AZURE_OPENAI_DEPLOYMENT_REASON01" "${AZURE_OPENAI_DEPLOYMENT_REASON01}"
  append_replacement "AZURE_OPENAI_DEPLOYMENT_REASON02" "${AZURE_OPENAI_DEPLOYMENT_REASON02:-}"
  append_replacement "AZURE_OPENAI_DEPLOYMENT_REASON03" "${AZURE_OPENAI_DEPLOYMENT_REASON03:-}"
